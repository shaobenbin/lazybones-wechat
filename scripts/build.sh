...
ignore_list="scripts|\.idea|.*\.md|.*\.yml"
...
electron-packager . "${APP_NAME}" --platform=$PLATFORM --arch=$ARCH --electronVersion=2.0.0 --app-version=1.0.0 --asar --icon=assets/icon.icns --overwrite --out=./build --ignore=${ignore_list}
if [ $? -eq 0 ]; then
